...
# Install required app (NTP - keep time correct, FEH - Picture Frame app, RSYNC - to copy pictures)
apt install ntp feh rsync -y
#xscreensaver
...
sudo apt install ./rustdesk-1.1.9-raspberry-armhf.deb -y
...
search="exec /usr/lib/xorg/Xorg -nocursor $DISPLAY $XAUTHORITY"
replace="exec /usr/lib/xorg/Xorg -dpms -nocursor $DISPLAY $XAUTHORITY"
...
# User Note
echo Please run "google-drive-ocamlfuse" from the terminal on your linux desktop.
